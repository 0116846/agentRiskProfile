...
const App = {
     ...
     */
    _parseInWorker: function(text) {
        var self = this;
        return new Promise(function(resolve, reject) {
            var worker;
            try {
                worker = new Worker('js/jsonWorker.js');
            } catch (e1) {
                reject(new Error('Cannot create worker: ' + e1.message));
                return;
            }
            ...
            worker.onmessage = function(e) {
                if (e.data.type === 'status') {
                    self.showStatus(e.data.message, 'info');
            ...
            worker.postMessage({ type: 'parseText', text: text });
        });
    },
    ...
    loadFromUpload: async function(file) {
        ...
        this.showStatus(`Loading ${file.name} (${sizeMB} MB)...`, 'info');

        try {
            if (file.name.toLowerCase().endsWith('.json')) {
                this.showStatus(`Reading file (${sizeMB} MB)... This may take a moment.`, 'info');
                var text = await file.text();
                var strippedData;
                try {
                    this.showStatus('Parsing in background thread...', 'info');
                    strippedData = await this._parseInWorker(text);
                    text = null;
                } catch (workerErr) {
                    console.warn('Worker failed, falling back to main thread:', workerErr.message);
                    this.showStatus('Parsing JSON data...', 'info');
                    await new Promise(function(r) { setTimeout(r, 50); });
                    var json = JSON.parse(text);
                    text = null;
                    strippedData = null;
                    this.showStatus('Processing and optimizing data...', 'info');
                    await new Promise(function(r) { setTimeout(r, 50); });
                    var data = JsonDataLoader.loadFromObject(json);
                    json = null;
                    this.state.isJsonSource = true;
                    this.state.rawJsonData = JsonDataLoader._strippedData;
                    DataParser.loadFromJSON(data);
                    this.processData(data);
                    this.showStatus('Loaded ' + file.name + ' successfully', 'success');
                    return;
                }
                this.showStatus('Building dashboard...', 'info');
                await new Promise(function(r) { setTimeout(r, 50); });
                JsonDataLoader._strippedData = strippedData;
                JsonDataLoader.rawData = strippedData;
                var data = JsonDataLoader.parse(strippedData);
                this.state.isJsonSource = true;
                this.state.rawJsonData = strippedData;
                DataParser.loadFromJSON(data);
                this.processData(data);
            } else {
    ...
    loadFromPath: async function(path) {
        ...
        try {
            if (path.toLowerCase().endsWith('.json')) {
                this.showStatus('Fetching JSON data...', 'info');
                var response = await fetch(path);
                if (!response.ok) throw new Error('Failed to fetch: ' + response.status);
                var text = await response.text();
                var sizeMB = (text.length / (1024 * 1024)).toFixed(1);
                var strippedData;
                try {
                    this.showStatus('Parsing ' + sizeMB + ' MB in background thread...', 'info');
                    strippedData = await this._parseInWorker(text);
                    text = null;
                } catch (workerErr) {
                    console.warn('Worker failed, falling back:', workerErr.message);
                    this.showStatus('Parsing JSON data...', 'info');
                    await new Promise(function(r) { setTimeout(r, 50); });
                    var json = JSON.parse(text);
                    text = null;
    ...
    applyDateFilter: function() {
        ...
        const dateRange = (this.state.isJsonSource && JsonDataLoader._strippedData)
    ...
    updateSummaryCards: function(data) {
        ...
        var age48Count = data.age48hrs?.filter(r => r.age > 48).length || 0;
    ...
    updateManagerFocus: function() {
        ...
        container.innerHTML = managersNeedingFocus.slice(0, 6).map((m, index) => {
            ...
            sections.forEach(function(s) {
                if (ba[s.key] && ba[s.key].length) {
                    cleanNames(ba[s.key]).forEach(function(n) { allUniqueAgents.add(n); });
                }
            });
            ...
            popHtml += '</div>';
            var uniqueAgentCount = allUniqueAgents.size || agentCount;
            const escapedContent = popHtml.replace(/"/g, '&quot;');
            const escapedTitle = (m.manager + ' — ' + uniqueAgentCount + ' Unique Agents').replace(/"/g, '&quot;');
                    ...
                    <div class="card h-100 border-${focusClass} manager-focus-card"
                        ...
                        <div class="card-body text-center p-3">
                            ...
                            <div class="small mb-2 manager-agent-popover"
                                ...
                                <strong>${uniqueAgentCount} agents</strong> <span class="text-muted">(${cleanAgents.slice(0, 2).join(', ')}${cleanAgents.length > 2 ? '...' : ''})</span>
                            </div>
                            <div class="small">
                                ${breakdownHtml}
                            </div>
        ...
        container.querySelectorAll('.manager-agent-popover').forEach(el => {
            ...
            el.addEventListener('shown.bs.popover', function() {
                ...
                if (tip) {
                    var hdr = tip.querySelector('.popover-header');
                    if (hdr && !hdr.querySelector('.mgr-pop-close')) {
