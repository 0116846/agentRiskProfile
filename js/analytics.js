...
const Analytics = {
        ...
        Object.entries(byAgent).forEach(([agent, count]) => {
            ...
            if (zScore > this.thresholds.zScoreAnomaly) {
                anomalies.push({
                    ...
                    zScore,
                    severity: 'high',
                    percentAboveAvg: ((count - stats.mean) / stats.mean * 100).toFixed(1)
                });
    ...
    analyzeShortCalls: function(data) {
        ...
        const agentTotals = {};
        data.forEach(r => {
            const agentName = (r.agentName || '').trim().toUpperCase();
            if (!agentName) return;
            
            if (!agentTotals[agentName]) {
                agentTotals[agentName] = { agentName: r.agentName, count: 0 };
            }
            agentTotals[agentName].count += r.count || 0;
        });
        ...
        const aggregatedRankings = aggregatedData.map(r => {
            ...
            const percentOfAvg = (r.count / stats.mean * 100).toFixed(1);
        ...
        const rawRankings = data.map(r => {
            ...
            return {
                ...r,
                zScore,
                percentOfAvg: (r.count / stats.mean * 100).toFixed(1),
    ...
    analyzeRonaTrend: function(data) {
        ...
        data.forEach(r => {
            ...
            if (!agentTotals[agentName]) {
                agentTotals[agentName] = { agentName: r.agentName, rona: 0 };
            }
            agentTotals[agentName].rona += r.rona || 0;
        });
        ...
        const aggregatedRankings = aggregatedData.map(r => {
            ...
            const percentOfAvg = (r.rona / stats.mean * 100).toFixed(1);
            const isOutlier = zScore > this.thresholds.zScoreAnomaly;
            const isWarning = zScore > this.thresholds.zScoreWarning;
        ...
        const rawRankings = data.map(r => {
            ...
            return {
                ...r,
                zScore,
                percentOfAvg: (r.rona / stats.mean * 100).toFixed(1),
                status: zScore > this.thresholds.zScoreAnomaly ? 'outlier' : 
                       (zScore > this.thresholds.zScoreWarning ? 'warning' : 'normal'),
                vsAverage: r.rona - stats.mean
            };
        ...
        if (data.shortCalls?.length) {
            ...
            shortCallsAnalysis.outliers.forEach(o => {
                anomalies.push({
                    ...
                    message: `${o.agentName} has ${o.count} short calls (${o.percentOfAvg}% of average)`
                });
            });
        }
        ...
        if (data.ronaTrend?.length) {
            ...
            ronaAnalysis.outliers.forEach(o => {
                anomalies.push({
                    ...
                    value: o.rona,
                    severity: o.status === 'outlier' ? 'high' : 'medium',
                    message: `${o.agentName} has RONA of ${o.rona} (${o.percentOfAvg}% of average)`
                });
            });
        }
        ...
        if (data.emailChanges?.length) {
            ...
            emailAnalysis.anomalies.forEach(a => {
                anomalies.push({
                    ...
                    value: a.count,
                    severity: a.severity,
                    message: `${a.agent} made ${a.count} email changes (${a.percentAboveAvg}% above average)`
                });
            });
        }
    ...
    calculateAgentRiskScores: function(data) {
        ...
        const isValidAgent = (agent) => {
            ...
            const upperAgent = agent.toUpperCase().trim();
            return upperAgent !== '' && upperAgent !== 'TOTAL' && upperAgent !== 'GRAND TOTAL';
        };
        
        // Collect and SUM short calls data (agent may appear multiple times)
        if (data.shortCalls?.length) {
            data.shortCalls.forEach(row => {
                const agent = row.agentName ? String(row.agentName).trim().toUpperCase() : '';
        ...
        // Collect and SUM RONA data (agent may appear multiple times)
        if (data.ronaTrend?.length) {
        ...
        // Count email changes per agent
        if (data.emailChanges?.length) {
            data.emailChanges.forEach(row => {
                const agent = row.editedBy ? String(row.editedBy).trim().toUpperCase() : '';
                if (!isValidAgent(agent)) return;
                ensureAgent(agent);
        ...
        // Count aged cases (>48 hrs) per agent
        if (data.age48hrs?.length) {
            data.age48hrs.forEach(row => {
                const agent = row.caseOwner ? String(row.caseOwner).trim().toUpperCase() : '';
                if (!isValidAgent(agent)) return;
                ensureAgent(agent);
                if (row.age > 48) {
