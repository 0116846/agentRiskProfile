...
const App = {
    ...
    updateSummaryCards: function(data) {
        ...
        document.querySelectorAll('.summary-card').forEach(function(card) {
            card.removeEventListener('mouseenter', card._tipHandler);
            card._tipHandler = function() {
                if (card._tipBuilt) return;
                card._tipBuilt = true;
                var mb = self._buildMonthlyBreakdown(self._summaryData);
                var tipMap = { 'caseRecord-tab': mb.cases, 'proactive-tab': mb.proactive, 'age48-tab': mb.aged, 'emailChanges-tab': mb.email, 'shortCalls-tab': mb.shortCalls, 'rona-tab': mb.rona };
                document.querySelectorAll('.summary-card').forEach(function(c) {
                    var bucket = tipMap[c.getAttribute('data-tab')];
                    if (bucket) c.title = self._buildTooltipText(bucket);
                    c._tipBuilt = true;
                });
            };
            card._tipBuilt = false;
            card.addEventListener('mouseenter', card._tipHandler);
        });
    ...
    _renderCasesReport: function(data) {
        ...
        var statusColors = { 'Closed': '#2d6a4f', 'Open': '#e8651a', 'Escalated': '#d62828', 'New': '#457b9d', 'In Progress': '#f4a261', 'Pending': '#264653' };
        var fallbackColors = ['#6366f1', '#06b6d4', '#f59e0b', '#ef4444', '#10b981', '#8b5cf6', '#ec4899'];
            ...
            this._casesReportCharts.status = new Chart(statusCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['#e8651a', '#2d6a4f', '#d62828', '#457b9d', '#f4a261', '#264653', '#e76f51']
                    }]
                },
    ...
    updateProactiveTable: function(data) {
        ...
        if (ctx) {
            ...
            var colors = ['#6366f1', '#06b6d4', '#f59e0b', '#ef4444', '#10b981', '#8b5cf6', '#ec4899'];
    ...
    updateAgeTable: function(data) {
        ...
        tbody.innerHTML = sorted.slice(0, 500).map(row => {
            ...
            if (row.createdDate) { var d = new Date(row.createdDate); dateStr = isNaN(d) ? '' : d.toLocaleDateString('en-US', {month:'short',day:'numeric',year:'numeric'}); }
            return `
                <tr class="${row.status === 'critical' ? 'table-danger' : ''}">
                    <td>${row.caseNumber}</td>
                    <td>${row.caseOwner}</td>
                    <td>${row.manager}</td>
    ...
    updateShortCallsTable: function(data) {
        ...
        tbody.innerHTML = rankings.slice(0, 50).map(row => {
            ...
            return `
                <tr class="${row.status === 'outlier' ? 'table-danger' : ''}">
                    <td>${row.agentName}</td>
                    <td><strong>${row.count}</strong></td>
                    <td>${vsAvg} (${row.percentOfAvg}%)</td>
                    <td><span class="badge bg-${statusClass}">${statusText}</span></td>
                </tr>
            `;
        }).join('');
    ...
    renderLeaderboardRows: function() {
        ...
        tbody.innerHTML = rankings.slice(0, this._leaderboardDisplayCount).map((agent, index) => {
            ...
            const rankBadge = rank <= 3 ? 
                `<span class="badge bg-danger rounded-circle">${rank}</span>` : 
                `<span class="badge bg-secondary rounded-circle">${rank}</span>`;
            
            // Progress bar for risk score
            const progressColor = agent.riskClass === 'danger' ? '#dc3545' : 
                                 (agent.riskClass === 'warning' ? '#ffc107' : 
                                 (agent.riskClass === 'info' ? '#0dcaf0' : '#198754'));
            ...
            return `
                <tr class="${agent.riskLevel === 'Critical' ? 'table-danger' : (agent.riskLevel === 'High' ? 'table-warning' : '')}">
                    ...
                    <td><span class="badge bg-${agent.riskClass}">${agent.riskLevel}</span></td>
                </tr>
            `;
        }).join('');
    ...
    displayInsights: function(insights) {
        ...
        if (mgr || agt || dateLabel) {
            ...
            filterBanner = '<div class="col-12 mb-2"><div class="d-flex align-items-center gap-2 px-3 py-2 rounded-2 flex-wrap" ' +
                'style="background:linear-gradient(90deg,rgba(99,102,241,0.1),rgba(139,92,246,0.05));border:1px solid rgba(99,102,241,0.15);">' +
                '<i class="bi bi-funnel-fill text-primary"></i>' +
                '<span class="fw-semibold" style="color:#312e81;font-size:0.85rem;">Insights filtered: <strong>' + parts.join(' &bull; ') + '</strong></span>' +
    ...
    toggleTheme: function() {
        ...
        if (this.state.dataLoaded && this.currentAnalysis) {
            Charts.initializeAll(this.currentAnalysis, this.currentData);
        }
    },
    ...
    enableTableFilters: function(tableId) {
        ...
        headers.forEach(function(th, colIdx) {
            ...
            select.style.cssText = 'font-size:0.75rem; padding:2px 24px 2px 6px; border-radius:6px; border:1px solid #c4b5fd; color:#1e1b4b;';
    ...
    updateManagerFocus: function() {
        ...
        container.innerHTML = managersNeedingFocus.slice(0, 6).map((m, index) => {
            ...
            // Build rich per-metric popover content (HTML, selectable)
            const ba = m.breakdownAgents || {};
            const cleanNames = arr => arr.map(n => n.replace(/[\s.]+$/g, '').replace(/\s+/g, ' ')).filter(n => n.length > 1);
            const popoverLines = [];
            if (ba.agedCases?.length) { const names = cleanNames(ba.agedCases); popoverLines.push(`<div class="mb-1"><strong class="text-danger">Aged Cases</strong> — ${names.length} agents<br><span class="text-muted">${names.join(', ')}</span></div>`); }
            if (ba.shortCalls?.length) { const names = cleanNames(ba.shortCalls); popoverLines.push(`<div class="mb-1"><strong class="text-info">Short Calls</strong> — ${names.length} agents<br><span class="text-muted">${names.join(', ')}</span></div>`); }
            if (ba.rona?.length) { const names = cleanNames(ba.rona); popoverLines.push(`<div class="mb-1"><strong class="text-secondary">RONA</strong> — ${names.length} agents<br><span class="text-muted">${names.join(', ')}</span></div>`); }
            if (ba.emailChanges?.length) { const names = cleanNames(ba.emailChanges); popoverLines.push(`<div class="mb-1"><strong class="text-warning">Email Changes</strong> — ${names.length} agents<br><span class="text-muted">${names.join(', ')}</span></div>`); }
            const popoverContent = popoverLines.length ? popoverLines.join('') : cleanNames(m.agents).join(', ');
            const escapedContent = popoverContent.replace(/"/g, '&quot;');
        ...
        // Initialize Bootstrap popovers on agent count elements
