...
const Chatbot = {
    ...
    greeting: `Hi! I'm your **AI Dashboard Assistant**. I can answer *any* question about your agent data, drill down into details, and help you analyze performance.
...
- "Reset filters" / "Clear all" / "Yeet filters"
...
- "Tell me more" / "Why?" / "Drill down" / "Who?"`;
    ...
    handleAgentInfo: function (intent) {
        ...
        if (aboveAvg.length > 0) {
            r += `**Concerns:** ${aboveAvg.map(m => `${m.name} (${((m.value / m.avg - 1) * 100).toFixed(0)}% above avg)`).join(', ')}\n`;
        }
    ...
    handleEmailChanges: function (intent) {
        ...
        if (ec.anomalies.length > 0) {
            ...
            ec.anomalies.slice(0, 5).forEach((a, i) => {
                r += `${i + 1}. **${a.agent}**: ${a.count} changes (${a.percentAboveAvg}% above avg) - ${a.severity}\n`;
            });
        }
    ...
    handleInsightsForAgent: function (intent) {
        ...
        if (match.shortCalls > 0 && analysis?.shortCalls?.stats) {
            ...
            const pctAbove = avg > 0 ? ((match.shortCalls / avg - 1) * 100).toFixed(0) : 0;
            if (match.shortCalls > avg * 1.5) {
                findings.push({ severity: 'high', text: `Short Calls (**${match.shortCalls}**) is ${pctAbove}% above team average (${avg.toFixed(0)}). Needs call handling review.` });
        ...
        if (match.rona > 0 && analysis?.ronaTrend?.stats) {
            ...
            const pctAbove = avg > 0 ? ((match.rona / avg - 1) * 100).toFixed(0) : 0;
            if (match.rona > avg * 2) {
                findings.push({ severity: 'high', text: `RONA (**${match.rona}**) is ${pctAbove}% above team average (${avg.toFixed(1)}). Investigate availability.` });
    ...
    handleThemeSwitch: function(intent) {
        var theme = intent.theme || 'dark';
        document.documentElement.setAttribute('data-bs-theme', theme);
        localStorage.setItem('theme', theme);
        var icon = document.querySelector('#themeToggle i');
        if (icon) icon.className = theme === 'dark' ? 'bi bi-sun-fill' : 'bi bi-moon-fill';
        return theme === 'dark'
            ? "🌙 **Dark mode activated.** Easy on the eyes! Say \"light mode\" to switch back."
            : "☀️ **Light mode activated.** Bright and clear! Say \"dark mode\" to switch back.";
    }
};
