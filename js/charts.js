...
const Charts = {
    ...
    // Color palettes
    colors: {
        primary: ['#667eea', '#764ba2', '#66a6ff', '#89f7fe', '#a18cd1', '#fbc2eb'],
        danger: ['#f5576c', '#f093fb', '#ff6b6b', '#ee5253', '#ff7675'],
        success: ['#00f2fe', '#4facfe', '#20bf6b', '#26de81', '#0fb9b1'],
        warning: ['#fee140', '#fa709a', '#f7971e', '#ffd93d', '#f39c12'],
        neutral: ['#6c757d', '#adb5bd', '#dee2e6', '#495057', '#343a40'],
        mixed: [
            '#667eea', '#f5576c', '#00f2fe', '#fee140', '#a18cd1',
            '#ff6b6b', '#4facfe', '#fbc2eb', '#26de81', '#f7971e'
        ]
    },
        ...
        this.instances.ageDistribution = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: [
                        '#28a745', '#28a745', '#ffc107', '#ffc107', '#dc3545', '#dc3545'
                    ],
                    borderRadius: 8
                }]
            },
    ...
    createEmailChangesByAgentChart: function(analysis) {
        ...
        const colors = labels.map(label => 
            anomalyAgents.includes(label) ? '#dc3545' : '#667eea'
        );
    ...
    createShortCallsChart: function(analysis) {
        ...
        const colors = sortedData.map(r => {
            if (r.status === 'outlier') return '#dc3545';
    ...
    createCasesByOwnerChart: function(data) {
        ...
        this.instances.casesByOwner = new Chart(ctx, {
            type: 'bar',
            data: { labels: sorted.map(function(e) { return e[0]; }), datasets: [{ label: 'Cases', data: sorted.map(function(e) { return e[1]; }), backgroundColor: '#667eea', borderRadius: 6 }] },
    ...
    createCasesByManagerChart: function(data) {
        ...
        this.instances.casesByManager = new Chart(ctx, {
            type: 'bar',
            data: { labels: sorted.map(function(e) { return e[0]; }), datasets: [{ label: 'Cases', data: sorted.map(function(e) { return e[1]; }), backgroundColor: '#8b5cf6', borderRadius: 6 }] },
            options: { indexAxis: 'y', responsive: true, plugins: { legend: { display: false } }, scales: { x: { beginAtZero: true }, y: { ticks: { font: { size: 10 } } } } }
        });
    },
    ...
    createActivityTrendChart: function(data) {
        ...
        this.instances.activityTrend = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [
                    ds('Cases', 'cases', 'rgb(102,126,234)'),
                    ds('Aged Cases', 'aged', 'rgb(245,87,108)'),
                    ds('Email Changes', 'email', 'rgb(255,193,7)'),
                    ds('Short Calls', 'shortCalls', 'rgb(6,182,212)'),
                    ds('RONA', 'rona', 'rgb(168,85,247)')
                ]
            },
            options: {
                responsive: true,
